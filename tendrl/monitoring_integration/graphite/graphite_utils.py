...
def _add_metrics(objects, obj_name, metric, resource):
    ...
    for obj in objects[obj_name]["attrs"]:
        ...
        try:
                ...
                for key, value in resource[obj].items():
                    try:
                        ...
                        logger.log("error", NS.get("publisher_id", None),
                                   {'message': "Failed to fetch key {0} of attr {1}".format(key,obj) + str(ex)})
            else:
            ...
            logger.log("error", NS.get("publisher_id", None),
                      {'message': "Failed to fetch attribute {}".format(obj) + str(ex)})
            pass
...
def miscellaneous_metrics(cluster_details):
    metrics = []
    local_metrics = ["clusters.$integration_id.nodes.$node_name.brick_count.total.$brick_total_count",
                     "clusters.$integration_id.nodes.$node_name.brick_count.down.$brick_down_count",
                     "clusters.$integration_id.nodes.$node_name.brick_count.up.$brick_up_count"]
    for cluster_detail in cluster_details:
        for metric in local_metrics:
            ...
            for node in cluster_detail.details["Node"]:
                try:
                    local_metric = metric.replace("$node_name", node["fqdn"].replace(".", "_"))
                    ...
                    logger.log("error", NS.get("publisher_id", None),
                               {'message': "Failed to create brick metric for Node: {0} Metric: {1}".format(node, metric) + str(ex)})
    local_metrics = ["clusters.$integration_id.brick_count.total.$brick_total_count",
        ...
        for metric in local_metrics:
            ...
            for volume in cluster_detail.details["Volume"]:
                try:
                    ...
                    logger.log("error", NS.get("publisher_id", None),
                               {'message': "Failed to create volume metric {0} for Volume :{1}".format(metric, node) + str(ex)})
    local_metrics = ["clusters.$integration_id.nodes_count.total.$node_total_count",
        ...
        for metric in local_metrics:
            try:
                ...
                metrics.append(copy.deepcopy(local_metric))
                    ...
                    logger.log("error", NS.get("publisher_id", None),
                               {'message': "Failed to create cluster metric {0} for cluster {1}".format(metric, str(cluster_detail.integration_id)) + str(ex)})
    metric = "clusters.$integration_id.volumes.$volume_name.nodes.$node_name.bricks.$brick_name.status.$status"
    for cluster_detail in cluster_details:
        for brick in cluster_detail.details["Brick"]:
            try:
                local_metric = metric.replace("$integration_id", str(cluster_detail.integration_id))
                local_metric = local_metric.replace("$volume_name", brick["vol_name"])
                    ...
                    if "$node_name" in metric and "$brick_name" not in metric:
                        for node in cluster_detail.details["Node"]:
                            try:
                                ...
                                logger.log("error", NS.get("publisher_id", None),
                                           {'message': "Failed to fetch Node {} details ".format(node) + str(ex)})
                        ...
                        for volume in cluster_detail.details["Volume"]:
                            try:
                                ...
                                logger.log("error", NS.get("publisher_id", None),
                                           {'message': "Failed to fetch Volume {} details ".format(volume) + str(ex)})
